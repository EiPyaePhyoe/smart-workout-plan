...
router.get("/",checkAdmin,async function(req, res){
    ...
    res.render("admin/index",{workoutCount: workoutCount, challengeCount: challengeCount, foodCount:foodCount,recentFood: recentFood, recentWorkout: recentWorkout,});
});
router.get("/workoutadd", checkAdmin, function(req, res){
    res.render("admin/workout/add");
});
router.post("/workoutadd", checkAdmin, upload.single("cover"), async function(req, res){
...
router.get("/workoutlist", checkAdmin, async function(req, res){
    var workouts = await Workout.find({ createdBy: req.session.admin.id,
    isDeleted: false,
});
...
router.get("/workoutupdate/:id", checkAdmin, async function(req, res){
    var workout = await Workout.findOne({_id: req.params.id, createdBy: req.session.admin.id,});
...
router.post("/workoutupdate", checkAdmin, upload.single("cover"),async function(req, res){
    ...
    const data = await Workout.findOneAndUpdate(
        {_id: req.body.id, createdBy: req.session.admin.id}, {$set: update}
    );
    res.redirect("/admin/workoutlist");
});
...
router.post("/challengeadd", checkAdmin, uploadvideo.single("video"),async function(req, res){
...
router.get("/fooddelete/:id", checkAdmin, async function(req, res){
    ...
    if(data.image){
        try{
            fs.unlinkSync("public"+ challenge.image);
            }catch(e){
